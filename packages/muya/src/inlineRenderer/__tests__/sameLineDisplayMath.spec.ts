...
// KaTeX loads on demand in this fork; preload it so the boot render emits
// `.katex-display` synchronously instead of the source fallback.
beforeAll(async () => {
    await ensureKatex();
});
...
describe('same-line display math — interaction with the inlineMath toggle (#5004)', () => {
    it('keeps $$ display math when single-dollar inline math is disabled', () => {
        const tokens = tokenizer('Cost $5 now, $a$ and $$b$$ later', {
            options: { superSubScript: true, footnote: false, inlineMath: false },
        }).filter(
            (token): token is CodeEmojiMathToken => token.type === 'inline_math',
        );
    ...
    it('still tokenizes single-dollar math when the toggle is on', () => {
        const tokens = tokenizer('$a$ and $$b$$', {
            options: { superSubScript: true, footnote: false, inlineMath: true },
        }).filter(
            (token): token is CodeEmojiMathToken => token.type === 'inline_math',
        );
