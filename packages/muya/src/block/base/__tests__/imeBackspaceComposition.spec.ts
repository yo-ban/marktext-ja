...
// #4956 — pressing Backspace/Delete while an IME composition is active (e.g.
// deleting a candidate character mid-conversion) must reach the IME, not run
// the editor's backspaceHandler/deleteHandler. The composition text is not in
// `this.text` yet, so those handlers operate on a stale snapshot and dislocate
// the cursor. keydownHandler already guards Enter, the arrow keys and Tab with
// `!isComposed`; Backspace and Delete were missing the same guard.
...
describe('backspace/delete during IME composition (#4956)', () => {
