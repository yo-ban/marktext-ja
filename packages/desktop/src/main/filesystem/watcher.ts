...
class Watcher {
  ...
  private _watchUncFile(win: BrowserWindow, watchPath: string): () => void {
    ...
    const handler = async(): Promise<void> => {
      ...
      // The chokidar path runs every change through the self-write filter;
      // this fallback must too, or saving a file on a network share reports
      // the editor's own write as an external change.
      if (await this._shouldIgnoreEvent(win.id, watchPath, 'file', true)) {
        return
      }
