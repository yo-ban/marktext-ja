...
test.describe('Find bar — opening dismisses the inline format toolbar', () => {
  let app: ElectronApplication
  let page: Page

  test.beforeAll(async() => {
    const launched = await launchWithMarkdown('drag across this sentence to select it\n')
    app = launched.app
    page = launched.page
    await focusEditor(page)
  })
  ...
  // The toolbar's baseFloat parks itself off-screen instead of unmounting, so
  // shown/hidden is its wrapper's inline opacity (1 / 0), mirroring the muya
  // e2e float helpers.
  const toolbarOpacity = (page: Page): Promise<number> =>
    page.evaluate(() => {
      const picker = document.querySelector('.mu-format-picker')
      const wrapper = picker?.closest('.mu-float-wrapper') as HTMLElement | null
      return wrapper ? Number.parseFloat(wrapper.style.opacity || '0') : 0
    })
  ...
  test('a selection-raised format toolbar hides when Find opens', async() => {
    ...
    // and swallows clicks on the replace toggle.
    await openFind(app, page)
    await expect.poll(() => toolbarOpacity(page)).toBe(0)
    await closeAndReset(page)
  })
})
